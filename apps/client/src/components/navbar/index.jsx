...
const Navbar = () => {
  ...
  const isLinkActive = (path) => {
    return pathname === path ? "border-4 border-white rounded-full bg-blue-500 -mt-8 p-2" : "";
  };
    ...
    <main>
      <div className="fixed bottom-0 right-0 left-0 bg-blue-500 rounded-t-2xl p-4 lg:hidden">
        <ul className="flex justify-center gap-20 text-white">
          <Link href="/">
            <li className={`flex flex-col justify-center items-center ${isLinkActive("/")}`}>
              <House size={30} />
            </li>
          </Link>
          ...
          <Link href="/views/profile">
            <li className={`flex flex-col justify-center items-center ${isLinkActive("/views/profile")}`}>
              <User size={30} />
            </li>
          </Link>
        </ul>
